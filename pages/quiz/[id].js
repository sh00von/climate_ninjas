...
const QuizPage = () => {
  ...
  const quizQuestions = id
    ? quizData.find((quiz) => quiz.id === id)?.questions
    : []; // Find the quiz based on the id
  ...
  const handleAnswerSelect = (answerIndex) => {
    ...
    const isCorrect =
      answerIndex === quizQuestions[currentQuestion]?.correctAnswer; // Use optional chaining
    if (isCorrect) {
  ...
  const handleNextQuestion = () => {
    ...
    if (currentQuestion + 1 < quizQuestions.length) {
      ...
      // Update localStorage with completed quiz
      ...
      <div
        ...
        style={{
          background: `conic-gradient(
            #4f46e5 ${((30 - timer) / 30) * 100}%,
            #e5e7eb ${((30 - timer) / 30) * 100}%,
            #e5e7eb 100%
          )`,
        }}
      >
        <span className="text-4xl font-bold text-gray-900">{timer}</span>
      </div>
      <span className="mt-2 text-lg font-semibold text-gray-600">seconds</span>
    </div>
  </div>
);
        ...
        <main className="bg-white rounded-3xl p-8 shadow-lg flex flex-col items-center">
          {!quizCompleted ? (
            <div className="w-full">
              ...
              <div className="flex justify-center items-center mb-6">
                {quizQuestions.map((_, index) => (
                  <div
                    key={index}
                    className="flex items-center justify-center w-10 h-10 mx-2 rounded-full bg-gray-200"
                  >
            ...
            <div className="text-center p-6 bg-white ">
              ...
              <button
                onClick={resetQuiz}
                className="px-6 py-3 rounded-full bg-yellow-400 text-indigo-800 font-bold text-lg hover:bg-yellow-300 transition-colors duration-300 shadow-lg transform hover:scale-105"
              >
                🔄 Try Again
              </button>
            </div>
